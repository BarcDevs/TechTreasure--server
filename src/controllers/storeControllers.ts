...
import { AuthenticatedReq, UrlQuery } from '../types'
...
export const getItems =
  catchAsync(async (req: Request<any, any, UrlQuery>, res: Response, next: NextFunction) => {
...
export const submitRating = catchAsync(async (req: AuthenticatedReq, res: Response, next: NextFunction) => {
  const { id } = req.params
  const userId = req.user?._id
  const { rating } = req.body

  if (!userId)
    return next(new AppError(401, 'You must be logged in to submit a rating.'))
  ...
  const updatedItem = await updateItemById(id, { ratings: updatedRatings })

  if (!updatedItem)
    return next(new AppError(404, 'Item not updated. Please try again.'))

  successResponse(res, updatedItem, 200)
})
